...
contract TokenPair is ITokenPair, ERC20, ReentrancyGuard {
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        _setReserves(balance0, balance1);
        if (hasReward) kLast = reserveA * reserveB;
        emit Burn(BurnData({sender: msg.sender, amountA: amountA, amountB: amountB, to: to}));
    }
    ...
    function swap(
        ...
        _setReserves(balance0, balance1);
        emit Swap(SwapData({sender: msg.sender, amountAIn: amountAIn, amountBIn: amountBIn, amountAOut: amountAOut,  amountBOut: amountBOut, to: to}));
    }
